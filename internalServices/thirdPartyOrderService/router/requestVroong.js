...
function generateOrderNoteArgument(timeSlot) {
  const timeString = timeSlot.time_slot;
  return {
    order_notes: `${timeString}에 배송해주시면 됩니다.`,
  }
}

function requestVroong(orderIdx) {
  return Promise.all([
  ...
  .then((results) => {
    ...
    const orderNotesTimeArgument = generateOrderNoteArgument(timeSlot);

    const vroongOrderRequestContent = Object.assign(
      orderNotesTimeArgument,
